// ===== PLAN DATA =====
// Tập kết: 墨田区向島5-35-11 (nhà anh Hà)
// Khách sạn: HOTEL R9 The Yard Kurobe (2 đêm)
// Day 1 & 2: kết thúc tại hotel | Day 3: hotel → Tokyo tối

const PLANS = {
  A: {
    name: 'Alpine & 五箇山',
    color: '#2E4A6B',
    subtitle: 'Tường tuyết + UNESCO village + Trolley — ngày dài, trải nghiệm mạnh',
    route: '五箇山 → 室堂 → 黒部ダム → トロッコ',
    highlights: [
      { icon: '🏔️', text: 'Tường tuyết 15m tại Murodo' },
      { icon: '🏘️', text: '五箇山 UNESCO — yên tĩnh, ít đông' },
      { icon: '🚗', text: 'Tổng ~1,080km (3 ngày)' },
    ],
    routeGo: '上信越道 + 北陸道 (380km · 5h30) — cảnh núi→biển đẹp',
    routeReturn: '北陸道 → 関越道 (430km · 5h) — an toàn lái đêm, ít cua',
    routeOptions: [
      '✅ 上信越道 + 北陸道 — Ngắn nhất đến 五箇山. Cảnh núi→biển đẹp. (380km · 5h30)',
      '関越道 + 北陸道 via 長岡 — Dễ lái nhất, ít cua. Trẻ ít say. (430km · 6h)',
      '上信越道 xuyên Nagano — Cảnh núi đẹp nhất nhưng cua nhiều, trẻ dễ say. (360km · 5h30)',
    ],
    days: [
      {
        dayNum: 1,
        title: 'Day 1 — Chủ Nhật 3/5',
        subtitle: '向島 → 五箇山 → HOTEL R9',
        specialties: [
          { emoji: '🧈', name: '五箇山豆腐', where: '五箇山', desc: 'Đậu hũ cứng nướng than, đậm đà — đặc sản UNESCO village' },
          { emoji: '🐟', name: '岩魚塩焼き', where: '五箇山', desc: 'Cá suối nướng muối thơm giòn, trẻ em thích' },
          { emoji: '🍜', name: '富山ブラックラーメン', where: 'Kurobe', desc: 'Ramen đen xì dầu đậm đặc — ăn tối tại Kurobe' },
        ],
        segments: [
          { from: '向島', to: '五箇山', km: 380, time: '5h30', mode: 'car' },
          { from: '五箇山', to: 'HOTEL R9', km: 110, time: '1h30', mode: 'car' },
        ],
        stops: [
          {
            id: 'a-d1-depart',
            name: 'Tập kết nhà anh Hà (向島)',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '06:00',
            duration: 0,
            type: 'minor',
            description: '3 xe tập kết tại 墨田区向島5-35-11. Đổ xăng trên Meiji-dori → 向島入口 → 首都高6号 → 関越道.',
            food: null,
            tips: '2 nhà đến trước 5:50. Cùng xuất phát 6:00.',
            weather: '18-24°C · 🌤️ Tokyo ấm',
          },
          {
            id: 'gokayama',
            name: '五箇山 相倉集落',
            lat: 36.4260, lng: 136.9355,
            arrivalTime: '12:00',
            duration: 90,
            type: 'major',
            description: 'Làng mái tranh gassho UNESCO yên tĩnh nhất. Trẻ em thích khám phá nhà 3 tầng. Ít đông hơn 白川郷 nhiều.',
            food: '五箇山豆腐 · 岩魚塩焼き',
            tips: 'Exit 北陸道 tại 砺波IC → 東海北陸道 → 五箇山IC. Ghé trên đường, không vòng.',
            weather: '13-18°C · 🌤️ Ấm, có thể mưa nhẹ',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/b/ba/Ainokura_in_Gokayama_area.jpg/640px-Ainokura_in_Gokayama_area.jpg', alt: '五箇山 相倉集落 gassho village' },
            ],
          },
          {
            id: 'hotel-r9-a-d1',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '15:30',
            duration: 0,
            type: 'major',
            description: 'Check-in khách sạn. Nghỉ ngơi sau chặng đường dài.',
            food: 'Ăn tối tại nhà hàng quanh Kurobe',
            tips: 'Gần Kurobe IC, tiện di chuyển ngày hôm sau',
            weather: '14-20°C · 🌸 Mát mẻ buổi tối',
          },
        ],
      },
      {
        dayNum: 2,
        title: 'Day 2 — Thứ Hai 4/5',
        subtitle: '立山黒部 Alpine Route',
        specialties: [
          { emoji: '🍛', name: 'Kurobe Dam Curry', where: '黒部ダム', desc: 'Cà ri hình đập nước — món signature tại đập' },
          { emoji: '🍙', name: 'Bento trên núi', where: '室堂', desc: 'Mang bento ăn giữa tường tuyết — trải nghiệm độc đáo' },
        ],
        segments: [
          { from: 'HOTEL R9', to: '立山駅', km: 35, time: '45min', mode: 'car' },
          { from: '立山駅', to: '室堂', km: 23, time: '1h30', mode: 'alpine' },
          { from: '室堂', to: '黒部ダム', km: 10, time: '1h', mode: 'alpine' },
          { from: '黒部ダム', to: 'HOTEL R9', km: 90, time: '2h', mode: 'car' },
        ],
        stops: [
          {
            id: 'tateyama-sta',
            name: '立山駅',
            lat: 36.5835, lng: 137.4459,
            arrivalTime: '07:00',
            duration: 30,
            type: 'minor',
            description: 'Lên cáp treo bắt đầu Alpine Route. Đặt vé trước!',
            food: null,
            tips: 'Đến sớm xếp hàng, GW rất đông',
            weather: '10-15°C · 🌥️ Mát ở chân núi',
          },
          {
            id: 'murodo',
            name: '室堂・雪の大谷',
            lat: 36.5777, lng: 137.5949,
            arrivalTime: '09:00',
            duration: 120,
            type: 'major',
            description: 'Tường tuyết cao đến 15m! Điểm highlight của cả chuyến đi.',
            food: 'Bento mang theo hoặc quán tại Murodo',
            tips: 'Mặc áo ấm dày — nhiệt độ -3 đến 2°C. Tuyết có thể rơi.',
            weather: '-3 ~ 2°C · ❄️ Cần áo ấm dày, tuyết có thể rơi',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/da/Tateyama_Toll_Road_and_Yuki-no-otani.jpg/640px-Tateyama_Toll_Road_and_Yuki-no-otani.jpg', alt: '雪の大谷 tường tuyết' },
            ],
          },
          {
            id: 'kurobe-dam',
            name: '黒部ダム',
            lat: 36.5666, lng: 137.6629,
            arrivalTime: '13:00',
            duration: 90,
            type: 'major',
            description: 'Đập nước lớn nhất Nhật Bản giữa núi non hùng vĩ.',
            food: 'Kurobe Dam Curry (cà ri hình đập)',
            tips: 'Chụp ảnh panorama từ đài quan sát. Không vội — tận hưởng!',
            weather: '5-10°C · 🌬️ Lạnh, gió mạnh',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/a/a8/Kurobe_Dam_20141012_130913.jpg/640px-Kurobe_Dam_20141012_130913.jpg', alt: '黒部ダム Kurobe Dam' },
            ],
          },
          {
            id: 'hotel-r9-a-d2',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '16:30',
            duration: 0,
            type: 'minor',
            description: 'Về khách sạn nghỉ ngơi sau ngày trên núi.',
            food: 'Ăn tối: 富山ブラックラーメン hoặc hải sản quanh Kurobe',
            tips: 'Nghỉ sớm, mai có trolley!',
            weather: '14-20°C · 🌙 Mát mẻ',
          },
        ],
      },
      {
        dayNum: 3,
        title: 'Day 3 — Thứ Ba 5/5',
        subtitle: 'トロッコ → 宇奈月 → Nghỉ → Về Tokyo tối',
        specialties: [
          { emoji: '🍦', name: 'Kem onsen', where: '宇奈月', desc: 'Kem mềm mịn bán dọc phố onsen — trẻ em mê' },
          { emoji: '🍡', name: '温泉たまご', where: '宇奈月', desc: 'Trứng luộc suối nóng, vỏ mềm lòng đào — ăn kèm muối' },
          { emoji: '🍣', name: 'ます寿し bento', where: 'SA đường về', desc: 'Sushi cá hồi ép truyền thống Toyama — mua mang đi ăn trên xe' },
        ],
        segments: [
          { from: 'HOTEL R9', to: '宇奈月駅 (トロッコ)', km: 20, time: '25min', mode: 'car' },
          { from: 'トロッコ往復', to: 'Kanetsuri', km: 20, time: '1h30', mode: 'trolley' },
          { from: '宇奈月', to: 'HOTEL R9', km: 20, time: '25min', mode: 'car' },
          { from: 'HOTEL R9', to: '向島 (Tokyo)', km: 430, time: '5h', mode: 'car' },
        ],
        stops: [
          {
            id: 'kurobe-trolley-a',
            name: '黒部峡谷トロッコ',
            lat: 36.8150, lng: 137.5860,
            arrivalTime: '08:30',
            duration: 120,
            type: 'major',
            description: 'Tàu trolley dọc hẻm núi Kurobe. Đi short trip đến Kanetsuri (~40 phút) rồi quay lại.',
            food: 'Ăn sáng tại hotel trước khi đi',
            tips: 'Chọn toa mở để ngắm cảnh. Đặt vé trước! Chuyến sớm ít đông.',
            weather: '14-20°C · 🌿 Hẻm núi mát mẻ',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e8/Kurobe_Gorge_Railway_07.jpg/640px-Kurobe_Gorge_Railway_07.jpg', alt: '黒部峡谷トロッコ trolley' },
            ],
          },
          {
            id: 'a-d3-unazuki',
            name: '宇奈月温泉 散策',
            lat: 36.8173, lng: 137.5841,
            arrivalTime: '11:00',
            duration: 60,
            type: 'minor',
            description: 'Tản bộ phố onsen, ngâm chân ashiyu miễn phí. Mua omiyage.',
            food: 'Ăn trưa tại Unazuki',
            tips: 'Nhiều ashiyu dọc phố. Trẻ em rất thích!',
            weather: '14-20°C · 🌸 Dễ chịu',
          },
          {
            id: 'a-d3-rest',
            name: 'Về HOTEL R9 · Nghỉ ngơi & đóng đồ',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '13:00',
            duration: 0,
            type: 'minor',
            description: 'Về hotel nghỉ trưa, đóng đồ, cho trẻ ngủ. Tránh Uターンラッシュ chiều (13:00-18:00).',
            food: null,
            tips: '⚠️ KHÔNG lái xe 13:00-18:00 — giờ tắc nặng nhất GW',
            weather: '15-22°C · 🌤️',
          },
          {
            id: 'a-d3-return',
            name: 'Xuất phát về Tokyo',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '19:00',
            duration: 0,
            type: 'major',
            description: 'Về qua 北陸道 → 関越道 (an toàn lái đêm, ít cua). ~5h. Dự kiến về 向島 0:00-1:00 AM.',
            food: 'Mua bento/onigiri tại konbini trước khi đi',
            tips: 'Nghỉ SA nếu buồn ngủ. Chia ca lái. Trẻ sẽ ngủ trên xe.',
            weather: '14°C → 18°C · 🌙 Mát, đường vắng',
          },
        ],
      },
    ],
  },

  B: {
    name: 'Family Relax',
    color: '#3D6B4F',
    subtitle: 'Nhẹ nhàng, phù hợp trẻ rất nhỏ — không leo núi, không vội',
    route: '魚津水族館 → トロッコ → 宇奈月 → ヒスイ海岸',
    highlights: [
      { icon: '👶', text: 'Phù hợp mọi lứa tuổi, cả bé nhỏ' },
      { icon: '♨️', text: 'Onsen, bãi biển, thủy cung' },
      { icon: '🚗', text: 'Tổng ~960km (3 ngày)' },
    ],
    routeGo: '関越道 + 北陸道 via 長岡 (400km · 5h30) — dễ lái nhất, trẻ ngủ ngon',
    routeReturn: '北陸道 → 関越道 (400km · 5h) — an toàn lái đêm',
    routeOptions: [
      '✅ 関越道 + 北陸道 via 長岡 — Dễ lái nhất, ít cua. SA lớn. Trẻ ngủ ngon. (400km · 5h30)',
      '上信越道 + 北陸道 — Ngắn hơn 30km. Cảnh núi→biển đẹp. (370km · 5h)',
      '上信越道 xuyên Nagano — Ngắn nhất nhưng cua núi nhiều, không khuyên cho trẻ nhỏ. (350km · 5h)',
    ],
    days: [
      {
        dayNum: 1,
        title: 'Day 1 — Chủ Nhật 3/5',
        subtitle: '向島 → 魚津水族館 → HOTEL R9',
        specialties: [
          { emoji: '🦐', name: '白エビ天丼', where: '魚津', desc: 'Tôm trắng Toyama — đặc sản #1, giòn ngọt tự nhiên' },
          { emoji: '🦑', name: 'ほたるいか (Hotaruika)', where: '魚津', desc: 'Mực phát sáng Toyama Bay — sashimi hoặc tempura' },
          { emoji: '🍜', name: '富山ブラックラーメン', where: 'Kurobe', desc: 'Ramen đen xì dầu đậm đặc — ăn tối tại Kurobe' },
        ],
        segments: [
          { from: '向島', to: 'Kurobe area', km: 400, time: '5h30', mode: 'car' },
          { from: 'Kurobe', to: '魚津水族館', km: 20, time: '25min', mode: 'car' },
          { from: '魚津', to: 'HOTEL R9', km: 20, time: '25min', mode: 'car' },
        ],
        stops: [
          {
            id: 'b-d1-depart',
            name: 'Tập kết nhà anh Hà (向島)',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '06:00',
            duration: 0,
            type: 'minor',
            description: '3 xe tập kết tại 墨田区向島5-35-11. Đổ xăng → 向島入口 → 首都高 → 関越道.',
            food: null,
            tips: '2 nhà đến trước 5:50. Cùng xuất phát 6:00.',
            weather: '18-24°C · 🌤️ Tokyo ấm',
          },
          {
            id: 'uozu-aqua',
            name: '魚津水族館 + ミラージュランド',
            lat: 36.7984, lng: 137.3882,
            arrivalTime: '12:30',
            duration: 120,
            type: 'major',
            description: 'Thủy cung cổ nhất Nhật Bản — nhỏ nhưng ấm cúng, bé nào cũng thích. Bên cạnh là ミラージュランド (đu quay, rides nhẹ cho trẻ).',
            food: 'Ăn trưa tại khu vực / mang bento',
            tips: 'Xem show cá (giờ cố định). ミラージュランド rides ~300-400¥/lượt.',
            weather: '15-22°C · 🌤️ Ấm, ven biển gió mát',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c4/UOZU_AQUARIUM.jpg/640px-UOZU_AQUARIUM.jpg', alt: '魚津水族館' },
            ],
          },
          {
            id: 'hotel-r9-b-d1',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '15:00',
            duration: 0,
            type: 'major',
            description: 'Check-in sớm, nghỉ ngơi. Chiều thoải mái.',
            food: 'Ăn tối: hải sản tươi Toyama',
            tips: 'Gần Kurobe IC, yên tĩnh',
            weather: '14-20°C · 🌸 Mát mẻ',
          },
        ],
      },
      {
        dayNum: 2,
        title: 'Day 2 — Thứ Hai 4/5',
        subtitle: 'トロッコ → 宇奈月温泉 → ヒスイ海岸',
        specialties: [
          { emoji: '🍦', name: 'Kem onsen', where: '宇奈月', desc: 'Kem mềm mịn bán dọc phố onsen' },
          { emoji: '🍡', name: '温泉たまご', where: '宇奈月', desc: 'Trứng luộc suối nóng, lòng đào mềm' },
          { emoji: '🍘', name: 'Bánh senbei nướng', where: '宇奈月', desc: 'Trẻ em tự nướng tại quầy ven phố' },
        ],
        segments: [
          { from: 'HOTEL R9', to: 'トロッコ乗場', km: 20, time: '25min', mode: 'car' },
          { from: 'トロッコ往復', to: 'Kanetsuri', km: 20, time: '2h', mode: 'trolley' },
          { from: '宇奈月', to: 'ヒスイ海岸', km: 25, time: '30min', mode: 'car' },
          { from: 'ヒスイ海岸', to: 'HOTEL R9', km: 25, time: '30min', mode: 'car' },
        ],
        stops: [
          {
            id: 'kurobe-trolley-b',
            name: '黒部峡谷トロッコ',
            lat: 36.8150, lng: 137.5860,
            arrivalTime: '09:00',
            duration: 120,
            type: 'major',
            description: 'Tàu trolley qua hẻm núi Kurobe — highlight cho trẻ em! Đi đến Kanetsuri rồi quay lại.',
            food: 'Ăn sáng tại hotel, mang đồ ăn nhẹ',
            tips: 'Đặt vé trước! Chọn toa mở (open car). Mang áo khoác — hẻm núi mát.',
            weather: '14-20°C · 🌿 Hẻm núi mát mẻ, cảnh xanh',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e8/Kurobe_Gorge_Railway_07.jpg/640px-Kurobe_Gorge_Railway_07.jpg', alt: '黒部峡谷トロッコ' },
            ],
          },
          {
            id: 'unazuki-onsen',
            name: '宇奈月温泉街',
            lat: 36.8173, lng: 137.5841,
            arrivalTime: '11:30',
            duration: 90,
            type: 'major',
            description: 'Tản bộ phố onsen, tắm chân (ashiyu) miễn phí dọc phố. Trẻ em chơi thoải mái.',
            food: 'Kem onsen · bánh mochi · ăn trưa tại phố',
            tips: 'Nhiều ashiyu miễn phí! Yamabiko Bridge view đẹp.',
            weather: '14-20°C · 🌸 Không khí trong lành',
          },
          {
            id: 'hisui-beach',
            name: 'ヒスイ海岸 (Bãi biển ngọc bích)',
            lat: 36.9456, lng: 137.5821,
            arrivalTime: '14:00',
            duration: 90,
            type: 'major',
            description: 'Bãi biển tìm đá ngọc bích (jade) thật! Trẻ em "săn kho báu" trên bãi đá — hoạt động ngoài trời tuyệt vời.',
            food: null,
            tips: 'Mang xô/túi. Đá jade màu xanh lục nhạt. Trẻ rất thích dù không tìm được jade thật!',
            weather: '15-20°C · 🌊 Gió biển mát, nắng nhẹ',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/2/2a/Itoigawa_Jade_Coast_ac_%281%29.jpg/640px-Itoigawa_Jade_Coast_ac_%281%29.jpg', alt: 'ヒスイ海岸 bãi biển ngọc bích' },
            ],
          },
          {
            id: 'hotel-r9-b-d2',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '16:00',
            duration: 0,
            type: 'minor',
            description: 'Về hotel sớm, nghỉ ngơi thoải mái.',
            food: 'Ăn tối: 富山ブラックラーメン',
            tips: 'Ngày nhẹ nhàng — trẻ không mệt!',
            weather: '14-20°C · 🌙 Mát',
          },
        ],
      },
      {
        dayNum: 3,
        title: 'Day 3 — Thứ Ba 5/5',
        subtitle: 'TAD + 環水公園 → Nghỉ → Về Tokyo tối',
        specialties: [
          { emoji: '🦐', name: '白エビバーガー', where: '環水公園', desc: 'Burger tôm trắng Toyama — bán tại quầy trong công viên' },
          { emoji: '☕', name: 'Starbucks 環水公園', where: '環水公園', desc: 'Chi nhánh đẹp nhất Nhật Bản — view kênh + Tateyama' },
          { emoji: '🍣', name: 'ます寿し', where: 'SA đường về', desc: 'Sushi cá hồi ép Toyama — bento hoàn hảo mang đi' },
        ],
        segments: [
          { from: 'HOTEL R9', to: 'TAD / 環水公園', km: 60, time: '50min', mode: 'car' },
          { from: 'TAD', to: 'HOTEL R9', km: 60, time: '50min', mode: 'car' },
          { from: 'HOTEL R9', to: '向島 (Tokyo)', km: 430, time: '5h', mode: 'car' },
        ],
        stops: [
          {
            id: 'b-d3-tad',
            name: '富山県美術館 TAD + 環水公園',
            lat: 36.7106, lng: 137.2101,
            arrivalTime: '09:30',
            duration: 120,
            type: 'major',
            description: 'Rooftop playground miễn phí — khu chơi sáng tạo cho trẻ + view Tateyama. Bên cạnh là 環水公園 tản bộ, Starbucks đẹp nhất Nhật Bản.',
            food: 'Cafe TAD · 白エビバーガー · Starbucks',
            tips: 'Hai nơi liền kề. Rooftop mở ngoài trời, miễn phí.',
            weather: '15-22°C · 🌤️ Ấm, view núi đẹp',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/53/Fugan_Canal_Kansui_Park_20170414.jpg/640px-Fugan_Canal_Kansui_Park_20170414.jpg', alt: '環水公園 Kansui Park' },
            ],
          },
          {
            id: 'b-d3-rest',
            name: 'Nghỉ ngơi & đóng đồ',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '12:30',
            duration: 0,
            type: 'minor',
            description: 'Về hotel nghỉ trưa, cho trẻ ngủ, đóng đồ. Tránh Uターンラッシュ chiều (13:00-18:00).',
            food: 'Ăn trưa quanh hotel',
            tips: '⚠️ KHÔNG lái xe 13:00-18:00 — giờ tắc nặng nhất GW',
            weather: '15-22°C · 🌤️',
          },
          {
            id: 'b-d3-return',
            name: 'Xuất phát về Tokyo',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '19:00',
            duration: 0,
            type: 'major',
            description: 'Về qua 北陸道 → 関越道 (an toàn lái đêm). ~5h. Dự kiến về 向島 0:00-1:00 AM.',
            food: 'Bento/onigiri mua tại konbini',
            tips: 'Nghỉ SA nếu buồn ngủ. Trẻ sẽ ngủ trên xe — mang chăn mỏng.',
            weather: '14°C → 18°C · 🌙 Mát, đường vắng',
          },
        ],
      },
    ],
  },

  C: {
    name: 'Matsumoto & Alpine',
    color: '#8B4513',
    subtitle: 'Nagano lâu đài + tường tuyết — đi/về khác đường, không đi lại',
    route: '松本城 → 室堂 → トロッコ → 魚津',
    highlights: [
      { icon: '🏯', text: '松本城 — quốc bảo Nhật Bản' },
      { icon: '🔄', text: 'Đi 中央道, về 関越道 — không trùng đường' },
      { icon: '🚗', text: 'Tổng ~1,050km (3 ngày)' },
    ],
    routeGo: '中央道 → 長野道 → 松本 → 大町/Hakuba (380km · 6h) — cảnh Alps đẹp nhất',
    routeReturn: '北陸道 → 関越道 (430km · 5h) — an toàn lái đêm, ít cua',
    routeOptions: [
      '✅ 中央道 → 長野道 → 松本 → 大町 → Kurobe — Cảnh Alps đẹp nhất! Thẳng Matsumoto. (380km · 6h)',
      '中央道 → 長野道 → 上信越道 → 北陸道 — Toàn expressway, nhanh hơn nhưng vòng. (420km · 5h30)',
      '関越道 → 上信越道 → 長野道 → 松本 — Alternative nhưng phải quay đầu. (400km · 6h)',
    ],
    days: [
      {
        dayNum: 1,
        title: 'Day 1 — Chủ Nhật 3/5',
        subtitle: '向島 → 松本城 → HOTEL R9',
        specialties: [
          { emoji: '🍜', name: '信州そば (Soba)', where: '松本', desc: 'Mì kiều mạch Nagano — ăn lạnh chấm nước tương, thơm nức' },
          { emoji: '🍎', name: 'リンゴソフト (Kem táo)', where: '松本', desc: 'Kem táo Shinshu — ngọt tự nhiên, bán quanh lâu đài' },
          { emoji: '🍶', name: 'Sake Nagano', where: 'SA trên đường', desc: 'Sake vùng Shinshu nổi tiếng — mua omiyage tại SA' },
        ],
        segments: [
          { from: '向島', to: '松本城', km: 230, time: '3h', mode: 'car' },
          { from: '松本城', to: 'HOTEL R9 (via 大町/Hakuba)', km: 150, time: '2h30', mode: 'car' },
        ],
        stops: [
          {
            id: 'c-d1-depart',
            name: 'Tập kết nhà anh Hà (向島)',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '06:00',
            duration: 0,
            type: 'minor',
            description: '3 xe tập kết tại 墨田区向島5-35-11. Đổ xăng → 首都高 → 中央道 hướng Matsumoto.',
            food: null,
            tips: '2 nhà đến trước 5:50. Plan C đi 中央道 (khác Plan A/B).',
            weather: '18-24°C · 🌤️ Tokyo ấm',
          },
          {
            id: 'matsumoto',
            name: '松本城',
            lat: 36.2387, lng: 137.9689,
            arrivalTime: '09:30',
            duration: 90,
            type: 'major',
            description: 'Lâu đài quạ đen — một trong 5 lâu đài quốc bảo. Hào nước + thiên nga, backdrop Alps tuyệt đẹp.',
            food: 'Soba Matsumoto (mì kiều mạch)',
            tips: 'Đến trước 10h để vào nhanh. GW sau 10h xếp hàng rất dài.',
            weather: '10-20°C · 🌤️ Mát buổi sáng, ấm trưa',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/f/f7/130608_Matsumoto_Castle_Matsumoto_Nagano_pref_Japan02bs4.jpg/640px-130608_Matsumoto_Castle_Matsumoto_Nagano_pref_Japan02bs4.jpg', alt: '松本城 Matsumoto Castle' },
            ],
          },
          {
            id: 'c-d1-drive',
            name: 'Lái xe qua Alps (大町 → Hakuba → Kurobe)',
            lat: 36.6500, lng: 137.8500,
            arrivalTime: '11:30',
            duration: 0,
            type: 'minor',
            description: 'Cung đường đẹp nhất chuyến đi! Qua thung lũng Alps, Hakuba ski resort, hồ Aoki. Không expressway — đường núi nhẹ nhàng.',
            food: null,
            tips: 'Route: 松本 → 大町 → 白馬 → 糸魚川 → Kurobe. Cảnh hai bên rất đẹp.',
            weather: '10-18°C · 🏔️ Núi tuyết hai bên đường',
          },
          {
            id: 'hotel-r9-c-d1',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '15:00',
            duration: 0,
            type: 'major',
            description: 'Check-in khách sạn. Nghỉ ngơi.',
            food: 'Ăn tối: hải sản tươi Toyama',
            tips: null,
            weather: '14-20°C · 🌙 Mát mẻ',
          },
        ],
      },
      {
        dayNum: 2,
        title: 'Day 2 — Thứ Hai 4/5',
        subtitle: 'Alpine Route (室堂) → トロッコ',
        specialties: [
          { emoji: '🍛', name: 'Kurobe Dam Curry', where: '室堂 area', desc: 'Cà ri hình đập — có bán tại quán trên Alpine Route' },
          { emoji: '🍙', name: 'おにぎり on the go', where: '室堂', desc: 'Onigiri mang lên núi — ăn giữa tường tuyết' },
          { emoji: '🍡', name: '温泉たまご', where: '宇奈月', desc: 'Trứng luộc suối nóng sau khi xuống trolley' },
        ],
        segments: [
          { from: 'HOTEL R9', to: '立山駅', km: 35, time: '45min', mode: 'car' },
          { from: '立山駅', to: '室堂', km: 23, time: '1h30', mode: 'alpine' },
          { from: '室堂', to: '立山駅 (復路)', km: 23, time: '1h30', mode: 'alpine' },
          { from: '立山駅', to: 'トロッコ乗場', km: 55, time: '1h', mode: 'car' },
          { from: 'トロッコ往復', to: 'Kanetsuri', km: 20, time: '2h', mode: 'trolley' },
          { from: 'トロッコ乗場', to: 'HOTEL R9', km: 20, time: '25min', mode: 'car' },
        ],
        stops: [
          {
            id: 'tateyama-sta-c',
            name: '立山駅',
            lat: 36.5835, lng: 137.4459,
            arrivalTime: '07:00',
            duration: 30,
            type: 'minor',
            description: 'Bắt đầu Alpine Route. Đặt vé trước!',
            food: null,
            tips: 'Đặt vé trước, GW sold out nhanh',
            weather: '10-15°C · 🌥️ Mát ở chân núi',
          },
          {
            id: 'murodo-c',
            name: '室堂・雪の大谷',
            lat: 36.5777, lng: 137.5949,
            arrivalTime: '09:00',
            duration: 120,
            type: 'major',
            description: 'Tường tuyết khổng lồ! Đỉnh cao của chuyến đi.',
            food: 'Bento hoặc quán Murodo',
            tips: 'Áo ấm dày bắt buộc. -3 đến 2°C, tuyết có thể rơi.',
            weather: '-3 ~ 2°C · ❄️ Rất lạnh, tuyết có thể rơi',
          },
          {
            id: 'kurobe-trolley-c',
            name: '黒部峡谷トロッコ',
            lat: 36.8150, lng: 137.5860,
            arrivalTime: '14:00',
            duration: 120,
            type: 'major',
            description: 'Tàu trolley hẻm núi Kurobe — cảnh đẹp tuyệt vời buổi chiều.',
            food: null,
            tips: 'Đi đến Kanetsuri rồi quay lại (~2h). Chọn toa mở!',
            weather: '14-20°C · 🌿 Hẻm núi xanh, mát mẻ',
          },
          {
            id: 'hotel-r9-c-d2',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '17:00',
            duration: 0,
            type: 'minor',
            description: 'Về khách sạn nghỉ ngơi.',
            food: 'Dinner tại nhà hàng quanh Kurobe',
            tips: null,
            weather: '14-20°C · 🌙 Mát',
          },
        ],
      },
      {
        dayNum: 3,
        title: 'Day 3 — Thứ Ba 5/5',
        subtitle: '魚津 + ヒスイ海岸 → Nghỉ → Về Tokyo tối',
        specialties: [
          { emoji: '🦐', name: '白エビ天丼', where: '魚津', desc: 'Tôm trắng Toyama chiên giòn — ăn trưa tại quán ven biển' },
          { emoji: '🦑', name: 'ほたるいか', where: '魚津', desc: 'Mực phát sáng — sashimi hoặc tempura, đặc sản Toyama Bay' },
          { emoji: '🍣', name: 'ます寿し', where: 'SA đường về', desc: 'Sushi cá hồi ép truyền thống — bento hoàn hảo trên xe' },
        ],
        segments: [
          { from: 'HOTEL R9', to: '魚津水族館', km: 20, time: '25min', mode: 'car' },
          { from: '魚津', to: 'ヒスイ海岸', km: 30, time: '35min', mode: 'car' },
          { from: 'ヒスイ海岸', to: 'HOTEL R9', km: 25, time: '30min', mode: 'car' },
          { from: 'HOTEL R9', to: '向島 (Tokyo)', km: 430, time: '5h', mode: 'car' },
        ],
        stops: [
          {
            id: 'c-d3-uozu',
            name: '魚津水族館 + ミラージュランド',
            lat: 36.7984, lng: 137.3882,
            arrivalTime: '09:00',
            duration: 90,
            type: 'major',
            description: 'Thủy cung nhỏ ấm cúng + công viên giải trí nhẹ bên cạnh. Đu quay nhìn ra biển Toyama.',
            food: null,
            tips: 'Xem show cá. ミラージュランド rides nhẹ cho trẻ.',
            weather: '15-22°C · 🌤️ Ấm, ven biển',
          },
          {
            id: 'c-d3-hisui',
            name: 'ヒスイ海岸 (Bãi biển ngọc bích)',
            lat: 36.9456, lng: 137.5821,
            arrivalTime: '11:00',
            duration: 60,
            type: 'major',
            description: 'Săn đá ngọc bích trên bãi biển! Hoạt động ngoài trời vui cho cả nhà.',
            food: null,
            tips: 'Ghé nhanh 1h. Mang xô cho trẻ.',
            weather: '15-20°C · 🌊 Gió biển mát',
          },
          {
            id: 'c-d3-rest',
            name: 'Nghỉ ngơi & đóng đồ',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '12:30',
            duration: 0,
            type: 'minor',
            description: 'Về hotel nghỉ trưa, đóng đồ. Tránh Uターンラッシュ chiều (13:00-18:00).',
            food: 'Ăn trưa quanh hotel',
            tips: '⚠️ KHÔNG lái xe 13:00-18:00 — giờ tắc nặng nhất GW',
            weather: '15-22°C · 🌤️',
          },
          {
            id: 'c-d3-return',
            name: 'Xuất phát về Tokyo',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '19:00',
            duration: 0,
            type: 'major',
            description: 'Về qua 北陸道 → 関越道 (khác đường đi 中央道 → cảnh mới!). ~5h. Về 向島 0:00-1:00 AM.',
            food: 'Bento/onigiri mua tại konbini',
            tips: 'Đi khác đường về → không nhàm chán. Nghỉ SA nếu buồn ngủ.',
            weather: '14°C → 18°C · 🌙 Mát, đường vắng',
          },
        ],
      },
    ],
  },

  D: {
    name: '五箇山 & 称名滝',
    color: '#5B6ABF',
    subtitle: 'Thác cao nhất Nhật + UNESCO village + Trolley — không cần vé Alpine, tiết kiệm',
    route: '五箇山 → 称名滝 → 魚津水族館 → トロッコ',
    highlights: [
      { icon: '💧', text: '称名滝 — thác cao nhất Nhật Bản (350m)' },
      { icon: '🏘️', text: '五箇山 UNESCO — yên tĩnh, ít đông' },
      { icon: '💰', text: 'Không cần vé Alpine Route — tiết kiệm ~¥55,000' },
    ],
    routeGo: '上信越道 + 北陸道 (380km · 5h30) — cảnh núi→biển đẹp',
    routeReturn: '北陸道 → 関越道 (430km · 5h) — an toàn lái đêm, ít cua',
    routeOptions: [
      '✅ 上信越道 + 北陸道 — Ngắn nhất đến 五箇山. Cảnh núi→biển đẹp. (380km · 5h30)',
      '関越道 + 北陸道 via 長岡 — Dễ lái nhất, ít cua. Trẻ ít say. (430km · 6h)',
      '上信越道 xuyên Nagano — Cảnh núi đẹp nhất nhưng cua nhiều, trẻ dễ say. (360km · 5h30)',
    ],
    days: [
      {
        dayNum: 1,
        title: 'Day 1 — Chủ Nhật 3/5',
        subtitle: '向島 → 五箇山 → HOTEL R9',
        specialties: [
          { emoji: '🧈', name: '五箇山豆腐', where: '五箇山', desc: 'Đậu hũ cứng nướng than, đậm đà — đặc sản UNESCO village' },
          { emoji: '🐟', name: '岩魚塩焼き', where: '五箇山', desc: 'Cá suối nướng muối thơm giòn, trẻ em thích' },
          { emoji: '🍜', name: '富山ブラックラーメン', where: 'Kurobe', desc: 'Ramen đen xì dầu đậm đặc — ăn tối tại Kurobe' },
        ],
        segments: [
          { from: '向島', to: '五箇山', km: 380, time: '5h30', mode: 'car' },
          { from: '五箇山', to: 'HOTEL R9', km: 110, time: '1h30', mode: 'car' },
        ],
        stops: [
          {
            id: 'd-d1-depart',
            name: 'Tập kết nhà anh Hà (向島)',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '06:00',
            duration: 0,
            type: 'minor',
            description: '3 xe tập kết tại 墨田区向島5-35-11. Đổ xăng trên Meiji-dori → 向島入口 → 首都高6号 → 関越道.',
            food: null,
            tips: '2 nhà đến trước 5:50. Cùng xuất phát 6:00.',
            weather: '18-24°C · 🌤️ Tokyo ấm',
          },
          {
            id: 'd-gokayama',
            name: '五箇山 相倉集落',
            lat: 36.4260, lng: 136.9355,
            arrivalTime: '12:00',
            duration: 90,
            type: 'major',
            description: 'Làng mái tranh gassho UNESCO yên tĩnh nhất. Trẻ em thích khám phá nhà 3 tầng. Ít đông hơn 白川郷 nhiều.',
            food: '五箇山豆腐 · 岩魚塩焼き',
            tips: 'Exit 北陸道 tại 砺波IC → 東海北陸道 → 五箇山IC. Ghé trên đường, không vòng.',
            weather: '13-18°C · 🌤️ Ấm, có thể mưa nhẹ',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/b/ba/Ainokura_in_Gokayama_area.jpg/640px-Ainokura_in_Gokayama_area.jpg', alt: '五箇山 相倉集落' },
            ],
          },
          {
            id: 'd-hotel-r9-d1',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '15:30',
            duration: 0,
            type: 'major',
            description: 'Check-in khách sạn. Nghỉ ngơi sau chặng đường dài.',
            food: 'Ăn tối tại nhà hàng quanh Kurobe',
            tips: 'Gần Kurobe IC, tiện di chuyển ngày hôm sau',
            weather: '14-20°C · 🌸 Mát mẻ buổi tối',
          },
        ],
      },
      {
        dayNum: 2,
        title: 'Day 2 — Thứ Hai 4/5',
        subtitle: '称名滝 → ほたるいか → 魚津水族館',
        specialties: [
          { emoji: '🦑', name: 'ほたるいか (Hotaruika)', where: 'ほたるいかミュージアム', desc: 'Mực phát sáng live — sashimi hoặc tempura, chỉ có mùa này' },
          { emoji: '🦐', name: '白エビ天丼', where: '魚津', desc: 'Tôm trắng Toyama chiên giòn — đặc sản #1' },
          { emoji: '🍦', name: 'Kem Tateyama', where: '称名滝 area', desc: 'Kem sữa tươi vùng núi — bán tại quầy gần bãi đỗ' },
        ],
        segments: [
          { from: 'HOTEL R9', to: '称名滝', km: 45, time: '50min', mode: 'car' },
          { from: '称名滝', to: 'ほたるいかミュージアム', km: 50, time: '50min', mode: 'car' },
          { from: 'ほたるいか', to: '魚津水族館', km: 10, time: '10min', mode: 'car' },
          { from: '魚津', to: 'HOTEL R9', km: 20, time: '25min', mode: 'car' },
        ],
        stops: [
          {
            id: 'd-shomyo',
            name: '称名滝 (Shomyo Falls)',
            lat: 36.5733, lng: 137.4867,
            arrivalTime: '08:00',
            duration: 120,
            type: 'major',
            description: 'Thác cao nhất Nhật Bản (350m)! Tháng 5 tuyết tan → thác Hannoki bên cạnh (500m) cũng chảy → double waterfall cực kỳ ấn tượng. Nắng sáng chiếu trực tiếp, có cầu vồng.',
            food: null,
            tips: 'Đi sáng sớm = đẹp nhất (nắng chiếu mặt thác + nước mạnh). Đi bộ 30 phút từ bãi đỗ, đường paved. Mang giày thoải mái.',
            weather: '10-16°C · 🌤️ Mát, nắng sáng đẹp',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/8/8b/Shomyo-daki_1.jpg/640px-Shomyo-daki_1.jpg', alt: '称名滝 Shomyo Falls' },
            ],
          },
          {
            id: 'd-hotaruika',
            name: 'ほたるいかミュージアム',
            lat: 36.7632, lng: 137.3355,
            arrivalTime: '11:00',
            duration: 60,
            type: 'major',
            description: 'Bảo tàng mực phát sáng — xem live hotaruika phát sáng trong phòng tối. Chỉ có mùa 3-5 mới có live display! Touch pool cho trẻ.',
            food: 'Ăn trưa: hotaruika tempura tại quán bên cạnh',
            tips: 'Live display là highlight — đừng bỏ lỡ! Trẻ em rất thích phòng tối xem mực sáng.',
            weather: '15-20°C · 🌤️ Ven biển, gió mát',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/7/77/Watasenia_scintillans.jpg', alt: 'ほたるいか hotaruika mực phát sáng' },
            ],
          },
          {
            id: 'd-uozu',
            name: '魚津水族館 + ミラージュランド',
            lat: 36.7984, lng: 137.3882,
            arrivalTime: '12:30',
            duration: 120,
            type: 'major',
            description: 'Thủy cung cổ nhất Nhật Bản — nhỏ nhưng ấm cúng. Bên cạnh là ミラージュランド (đu quay, rides nhẹ cho trẻ, view biển).',
            food: '白エビ天丼 tại quán ven biển',
            tips: 'Xem show cá (giờ cố định). ミラージュランド rides ~300-400¥/lượt. Trẻ chơi thoải mái.',
            weather: '15-22°C · 🌤️ Ấm, ven biển gió mát',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c4/UOZU_AQUARIUM.jpg/640px-UOZU_AQUARIUM.jpg', alt: '魚津水族館 Uozu Aquarium' },
            ],
          },
          {
            id: 'd-hotel-r9-d2',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '15:30',
            duration: 0,
            type: 'minor',
            description: 'Về hotel sớm, nghỉ ngơi thoải mái. Ngày nhẹ nhàng!',
            food: 'Ăn tối: hải sản tươi Toyama',
            tips: 'Về sớm — trẻ không mệt, người lớn thư giãn.',
            weather: '14-20°C · 🌙 Mát mẻ',
          },
        ],
      },
      {
        dayNum: 3,
        title: 'Day 3 — Thứ Ba 5/5',
        subtitle: 'トロッコ → 宇奈月 → Nghỉ → Về Tokyo tối',
        specialties: [
          { emoji: '🍦', name: 'Kem onsen', where: '宇奈月', desc: 'Kem mềm mịn bán dọc phố onsen — trẻ em mê' },
          { emoji: '🍡', name: '温泉たまご', where: '宇奈月', desc: 'Trứng luộc suối nóng, vỏ mềm lòng đào — ăn kèm muối' },
          { emoji: '🍣', name: 'ます寿し bento', where: 'SA đường về', desc: 'Sushi cá hồi ép truyền thống Toyama — mua mang đi ăn trên xe' },
        ],
        segments: [
          { from: 'HOTEL R9', to: '宇奈月駅 (トロッコ)', km: 20, time: '25min', mode: 'car' },
          { from: 'トロッコ往復', to: 'Kanetsuri', km: 20, time: '1h30', mode: 'trolley' },
          { from: '宇奈月', to: 'HOTEL R9', km: 20, time: '25min', mode: 'car' },
          { from: 'HOTEL R9', to: '向島 (Tokyo)', km: 430, time: '5h', mode: 'car' },
        ],
        stops: [
          {
            id: 'd-trolley',
            name: '黒部峡谷トロッコ',
            lat: 36.8150, lng: 137.5860,
            arrivalTime: '08:30',
            duration: 120,
            type: 'major',
            description: 'Tàu trolley dọc hẻm núi Kurobe. Đi short trip đến Kanetsuri (~40 phút) rồi quay lại.',
            food: 'Ăn sáng tại hotel trước khi đi',
            tips: 'Chọn toa mở để ngắm cảnh. Đặt vé trước! Chuyến sớm ít đông.',
            weather: '14-20°C · 🌿 Hẻm núi mát mẻ',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e8/Kurobe_Gorge_Railway_07.jpg/640px-Kurobe_Gorge_Railway_07.jpg', alt: '黒部峡谷トロッコ' },
            ],
          },
          {
            id: 'd-unazuki',
            name: '宇奈月温泉 散策',
            lat: 36.8173, lng: 137.5841,
            arrivalTime: '11:00',
            duration: 60,
            type: 'minor',
            description: 'Tản bộ phố onsen, ngâm chân ashiyu miễn phí. Mua omiyage.',
            food: 'Ăn trưa tại Unazuki',
            tips: 'Nhiều ashiyu dọc phố. Trẻ em rất thích!',
            weather: '14-20°C · 🌸 Dễ chịu',
          },
          {
            id: 'd-d3-rest',
            name: 'Về HOTEL R9 · Nghỉ ngơi & đóng đồ',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '13:00',
            duration: 0,
            type: 'minor',
            description: 'Về hotel nghỉ trưa, đóng đồ, cho trẻ ngủ. Tránh Uターンラッシュ chiều (13:00-18:00).',
            food: null,
            tips: '⚠️ KHÔNG lái xe 13:00-18:00 — giờ tắc nặng nhất GW',
            weather: '15-22°C · 🌤️',
          },
          {
            id: 'd-d3-return',
            name: 'Xuất phát về Tokyo',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '19:00',
            duration: 0,
            type: 'major',
            description: 'Về qua 北陸道 → 関越道 (an toàn lái đêm, ít cua). ~5h. Dự kiến về 向島 0:00-1:00 AM.',
            food: 'Mua bento/onigiri tại konbini trước khi đi',
            tips: 'Nghỉ SA nếu buồn ngủ. Chia ca lái. Trẻ sẽ ngủ trên xe.',
            weather: '14°C → 18°C · 🌙 Mát, đường vắng',
          },
        ],
      },
    ],
  },
  E: {
    name: 'Hakuba & Kurobe',
    color: '#C0392B',
    subtitle: 'Nghỉ dưỡng Hakuba + Trolley hẻm núi Kurobe — cảnh núi Alps tuyệt đẹp',
    route: '白馬 → 宇奈月 → トロッコ → 魚津',
    highlights: [
      { icon: '🏔️', text: 'Hakuba — thung lũng Alps Nhật Bản, gondola ngắm núi tuyết' },
      { icon: '🚂', text: 'トロッコ Kurobe — tàu hẻm núi must-do' },
      { icon: '🚗', text: 'Tổng ~980km (3 ngày)' },
    ],
    routeGo: '中央道 → 長野道 → 白馬 (270km · 4h) — cảnh Alps đẹp, đường tốt',
    routeReturn: '北陸道 → 関越道 (430km · 5h) — an toàn lái đêm, ít cua',
    routeOptions: [
      '✅ 中央道 → 長野道 → 白馬 — Nhanh nhất, cảnh Alps đẹp. (270km · 4h)',
      '関越道 → 上信越道 → 長野 → 白馬 — Ít cua hơn, dễ lái. (290km · 4h30)',
      '首都高 → 練馬IC → 関越道 — Tránh trung tâm Tokyo, SA lớn. (300km · 4h30)',
    ],
    days: [
      {
        dayNum: 1,
        title: 'Day 1 — Chủ Nhật 3/5',
        subtitle: '向島 → 白馬 · Nghỉ dưỡng thung lũng Alps',
        specialties: [
          { emoji: '🍖', name: 'Shinshu beef / soba', where: '白馬', desc: 'Thịt bò Shinshu mềm thơm & soba kiều mạch Nagano — đặc sản vùng' },
          { emoji: '🍺', name: 'Hakuba Brewing', where: '白馬', desc: 'Bia craft địa phương Hakuba — uống tối tại village' },
        ],
        segments: [
          { from: '向島', to: '白馬', km: 270, time: '4h', mode: 'car' },
        ],
        stops: [
          {
            id: 'e-d1-depart',
            name: 'Tập kết nhà anh Hà (向島)',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '06:00',
            duration: 0,
            type: 'minor',
            description: '3 xe tập kết tại 墨田区向島5-35-11. Đổ xăng → 向島入口 → 首都高 → 中央道.',
            food: null,
            tips: '2 nhà đến trước 5:50. Cùng xuất phát 6:00.',
            weather: '18-24°C · 🌤️ Tokyo ấm',
          },
          {
            id: 'e-d1-hakuba-gondola',
            name: '白馬八方尾根ゴンドラ',
            lat: 36.6983, lng: 137.8320,
            arrivalTime: '11:00',
            duration: 120,
            type: 'major',
            description: 'Gondola lên đỉnh ngắm toàn cảnh dãy Alps Bắc. Tuyết trắng đỉnh núi + thung lũng xanh bên dưới. Trẻ em phấn khích!',
            food: 'Ăn trưa tại nhà hàng trên núi hoặc quán dưới chân gondola',
            tips: 'Lên gondola sáng tránh đông. Mặc áo gió — trên đỉnh lạnh hơn 10°C so dưới.',
            weather: '5-15°C · 🏔️ Mát trên đỉnh, cần áo gió',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/4/43/Happo-one_in_Summer.jpg/640px-Happo-one_in_Summer.jpg', alt: '白馬八方尾根 Hakuba Happo-one' },
            ],
          },
          {
            id: 'e-d1-hakuba-village',
            name: '白馬村 散策',
            lat: 36.6984, lng: 137.8617,
            arrivalTime: '14:00',
            duration: 90,
            type: 'major',
            description: 'Tản bộ Hakuba Village — quán cafe, shop đồ outdoor, không khí thung lũng yên bình. Cảm giác resort Âu châu giữa Nhật Bản.',
            food: 'Cafe & bánh ngọt tại village',
            tips: 'Nhiều quán có terrace ngắm núi. Ghé cửa hàng The North Face Hakuba.',
            weather: '12-20°C · 🌤️ Ấm áp, trong lành',
          },
          {
            id: 'e-d1-hotel-hakuba',
            name: 'Nghỉ đêm tại Hakuba',
            lat: 36.7020, lng: 137.8610,
            arrivalTime: '16:00',
            duration: 0,
            type: 'major',
            description: 'Check-in hotel/pension tại Hakuba. Nghỉ ngơi, tắm onsen nếu có. Ăn tối tại village.',
            food: 'Tối: Shinshu soba + thịt bò nướng tại izakaya Hakuba',
            tips: 'Nhiều pension gia đình giá tốt ¥8,000-15,000/phòng. Đặt sớm — GW cháy phòng!',
            weather: '8-14°C · 🌙 Đêm núi se lạnh, dễ chịu',
          },
        ],
      },
      {
        dayNum: 2,
        title: 'Day 2 — Thứ Hai 4/5',
        subtitle: '白馬 → Kurobe · トロッコ hẻm núi',
        specialties: [
          { emoji: '🦐', name: '白エビ天丼', where: 'Kurobe/Uozu', desc: 'Tôm trắng Toyama — giòn ngọt, đặc sản #1 vùng' },
          { emoji: '🍜', name: '富山ブラックラーメン', where: 'Kurobe', desc: 'Ramen đen xì dầu đậm đặc — ăn tối tại Kurobe' },
        ],
        segments: [
          { from: '白馬', to: '宇奈月駅', km: 110, time: '2h', mode: 'car' },
          { from: 'トロッコ往復', to: 'Kanetsuri', km: 20, time: '1h30', mode: 'trolley' },
          { from: '宇奈月', to: 'HOTEL R9', km: 20, time: '25min', mode: 'car' },
        ],
        stops: [
          {
            id: 'e-d2-hakuba-checkout',
            name: 'Check-out Hakuba · Xuất phát',
            lat: 36.7020, lng: 137.8610,
            arrivalTime: '07:30',
            duration: 0,
            type: 'minor',
            description: 'Ăn sáng tại hotel, check-out. Lái qua đường núi Hakuba → 糸魚川 → 北陸道 → Kurobe.',
            food: 'Ăn sáng tại hotel',
            tips: 'Đường 148 xuyên Alps rất đẹp — chạy dọc sông, qua hầm. Trẻ thích!',
            weather: '10-16°C · 🌤️ Sáng sớm mát',
          },
          {
            id: 'e-d2-trolley',
            name: '黒部峡谷トロッコ',
            lat: 36.8150, lng: 137.5860,
            arrivalTime: '10:00',
            duration: 150,
            type: 'major',
            description: 'Tàu trolley dọc hẻm núi Kurobe — highlight của chuyến đi! Đi đến Kanetsuri (~40 phút), ăn trưa tại ga rồi quay lại.',
            food: 'Ăn trưa tại Kanetsuri (quán ven suối) hoặc bento mua tại ga',
            tips: 'Chọn toa mở để ngắm cảnh tối đa. ĐẶT VÉ TRƯỚC — GW cháy vé! Chuyến 10:00 vừa đẹp.',
            weather: '14-20°C · 🌿 Hẻm núi mát mẻ, cảnh tuyệt đẹp',
            images: [
              { url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e8/Kurobe_Gorge_Railway_07.jpg/640px-Kurobe_Gorge_Railway_07.jpg', alt: '黒部峡谷トロッコ trolley' },
            ],
          },
          {
            id: 'e-d2-unazuki',
            name: '宇奈月温泉 散策',
            lat: 36.8173, lng: 137.5841,
            arrivalTime: '13:00',
            duration: 60,
            type: 'minor',
            description: 'Tản bộ phố onsen sau khi xuống trolley. Ngâm chân ashiyu miễn phí, mua omiyage.',
            food: 'Kem onsen + trứng luộc suối nóng',
            tips: 'Nhiều ashiyu dọc phố. Trẻ em rất thích!',
            weather: '14-20°C · 🌸 Dễ chịu',
          },
          {
            id: 'e-d2-hotel-r9',
            name: 'HOTEL R9 The Yard Kurobe',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '15:00',
            duration: 0,
            type: 'major',
            description: 'Check-in khách sạn. Nghỉ ngơi, thư giãn sau ngày trolley.',
            food: 'Ăn tối: 白エビ天丼 hoặc 富山ブラックラーメン quanh Kurobe',
            tips: 'Gần Kurobe IC, tiện di chuyển ngày hôm sau',
            weather: '14-20°C · 🌸 Mát mẻ buổi tối',
          },
        ],
      },
      {
        dayNum: 3,
        title: 'Day 3 — Thứ Ba 5/5',
        subtitle: '魚津 → ヒスイ海岸 → Nghỉ → Về Tokyo tối',
        specialties: [
          { emoji: '🦑', name: 'ほたるいか (Hotaruika)', where: '魚津', desc: 'Mực phát sáng Toyama Bay — mùa 3-5月, sashimi hoặc boiled' },
          { emoji: '🍣', name: 'ます寿し bento', where: 'SA đường về', desc: 'Sushi cá hồi ép truyền thống Toyama — mua mang đi' },
        ],
        segments: [
          { from: 'HOTEL R9', to: '魚津水族館', km: 20, time: '25min', mode: 'car' },
          { from: '魚津', to: 'ヒスイ海岸', km: 40, time: '40min', mode: 'car' },
          { from: 'ヒスイ海岸', to: 'HOTEL R9', km: 35, time: '35min', mode: 'car' },
          { from: 'HOTEL R9', to: '向島 (Tokyo)', km: 430, time: '5h', mode: 'car' },
        ],
        stops: [
          {
            id: 'e-d3-uozu',
            name: '魚津水族館 + ミラージュランド',
            lat: 36.7984, lng: 137.3882,
            arrivalTime: '09:00',
            duration: 90,
            type: 'major',
            description: 'Thủy cung cổ nhất Nhật Bản — nhỏ ấm cúng. ミラージュランド đu quay nhìn ra biển Toyama.',
            food: null,
            tips: 'Xem show cá. ミラージュランド rides nhẹ ~300-400¥/lượt.',
            weather: '15-22°C · 🌤️ Ấm, ven biển',
          },
          {
            id: 'e-d3-hisui',
            name: 'ヒスイ海岸 (Bãi biển ngọc bích)',
            lat: 36.9456, lng: 137.5821,
            arrivalTime: '11:00',
            duration: 60,
            type: 'major',
            description: 'Săn đá ngọc bích trên bãi biển! Hoạt động ngoài trời vui cho cả nhà.',
            food: null,
            tips: 'Ghé nhanh 1h. Mang xô cho trẻ.',
            weather: '15-20°C · 🌊 Gió biển mát',
          },
          {
            id: 'e-d3-rest',
            name: 'Về HOTEL R9 · Nghỉ ngơi & đóng đồ',
            lat: 36.8719, lng: 137.4479,
            arrivalTime: '12:30',
            duration: 0,
            type: 'minor',
            description: 'Về hotel nghỉ trưa, đóng đồ, cho trẻ ngủ. Tránh Uターンラッシュ chiều (13:00-18:00).',
            food: 'Ăn trưa quanh hotel',
            tips: '⚠️ KHÔNG lái xe 13:00-18:00 — giờ tắc nặng nhất GW',
            weather: '15-22°C · 🌤️',
          },
          {
            id: 'e-d3-return',
            name: 'Xuất phát về Tokyo',
            lat: 35.7180, lng: 139.8130,
            arrivalTime: '19:00',
            duration: 0,
            type: 'major',
            description: 'Về qua 北陸道 → 関越道 (an toàn lái đêm, ít cua). ~5h. Dự kiến về 向島 0:00-1:00 AM.',
            food: 'Mua bento/onigiri tại konbini trước khi đi',
            tips: 'Nghỉ SA nếu buồn ngủ. Chia ca lái. Trẻ sẽ ngủ trên xe.',
            weather: '14°C → 18°C · 🌙 Mát, đường vắng',
          },
        ],
      },
    ],
  },
};

// ===== COMPARISON DATA =====
const COMPARISON = [
  { label: 'Tường tuyết', A: { text: 'Có', type: 'yes' }, B: { text: 'Không', type: 'no' }, C: { text: 'Có', type: 'yes' }, D: { text: 'Không', type: 'no' }, E: { text: 'Không', type: 'no' } },
  { label: 'Trolley hẻm núi', A: { text: 'Có', type: 'yes' }, B: { text: 'Có', type: 'yes' }, C: { text: 'Có', type: 'yes' }, D: { text: 'Có', type: 'yes' }, E: { text: 'Có', type: 'yes' } },
  { label: '称名滝 (350m)', A: { text: 'Không', type: 'no' }, B: { text: 'Không', type: 'no' }, C: { text: 'Không', type: 'no' }, D: { text: 'Có', type: 'yes' }, E: { text: 'Không', type: 'no' } },
  { label: 'Cần vé Alpine', A: { text: '~¥55,000', type: 'warn' }, B: { text: 'Không', type: 'yes' }, C: { text: '~¥55,000', type: 'warn' }, D: { text: 'Không', type: 'yes' }, E: { text: 'Không', type: 'yes' } },
  { label: 'Độ vất vả', A: { text: '🔥🔥🔥', type: 'neutral' }, B: { text: '🔥', type: 'neutral' }, C: { text: '🔥🔥🔥', type: 'neutral' }, D: { text: '🔥🔥', type: 'neutral' }, E: { text: '🔥🔥', type: 'neutral' } },
  { label: 'Bé < 4 tuổi', A: { text: '⚠️ Khó', type: 'warn' }, B: { text: 'Tốt', type: 'yes' }, C: { text: '⚠️ Khó', type: 'warn' }, D: { text: 'Tốt', type: 'yes' }, E: { text: 'Tốt', type: 'yes' } },
  { label: 'Tránh đông', A: { text: '⭐⭐', type: 'neutral' }, B: { text: '⭐⭐⭐', type: 'neutral' }, C: { text: '⭐⭐⭐', type: 'neutral' }, D: { text: '⭐⭐⭐', type: 'neutral' }, E: { text: '⭐⭐⭐', type: 'neutral' } },
  { label: 'Tổng km', A: { text: '~1,080km', type: 'neutral' }, B: { text: '~960km', type: 'neutral' }, C: { text: '~1,050km', type: 'neutral' }, D: { text: '~1,020km', type: 'neutral' }, E: { text: '~980km', type: 'neutral' } },
  { label: 'Điểm đặc biệt', A: { text: '五箇山 UNESCO', type: 'neutral' }, B: { text: 'ヒスイ海岸 săn ngọc', type: 'neutral' }, C: { text: '松本城 + Alps drive', type: 'neutral' }, D: { text: '称名滝 + 水族館', type: 'neutral' }, E: { text: '白馬 Alps + gondola', type: 'neutral' } },
];

// ===== CHECKLIST =====
const CHECKLIST = [
  { id: 'warm-jacket', text: 'Áo ấm dày (Murodo -5°C) — Plan A & C', critical: true },
  { id: 'snow-boots', text: 'Giày không trơn / giày đi tuyết — Plan A & C', critical: true },
  { id: 'ticket', text: 'Vé Alpine Route (ĐẶT TRƯỚC!) — Plan A & C', critical: true },
  { id: 'trolley-ticket', text: 'Vé Trolley Kurobe (ĐẶT TRƯỚC!)', critical: true },
  { id: 'sunglasses', text: 'Kính râm (tuyết phản chiếu)', critical: false },
  { id: 'sunscreen', text: 'Kem chống nắng (UV mạnh trên núi)', critical: false },
  { id: 'snacks', text: 'Đồ ăn nhẹ cho trẻ em', critical: false },
  { id: 'charger', text: 'Sạc dự phòng điện thoại', critical: false },
  { id: 'cash', text: 'Tiền mặt (nhiều nơi không nhận card)', critical: true },
  { id: 'rain-gear', text: 'Áo mưa nhẹ / ô gấp', critical: false },
  { id: 'motion-sick', text: 'Thuốc say xe cho trẻ', critical: false },
  { id: 'layers', text: 'Quần áo nhiều lớp (chênh lệch nhiệt)', critical: true },
  { id: 'bucket', text: 'Xô/túi nhỏ (săn đá ヒスイ海岸) — Plan B & C', critical: false },
  { id: 'blanket', text: 'Chăn mỏng cho trẻ ngủ xe (về đêm)', critical: false },
  { id: 'windbreaker', text: 'Áo gió cho Hakuba gondola (đỉnh lạnh) — Plan E', critical: false },
];

// ===== ESSENTIALS: LINKS =====
const USEFUL_LINKS = [
  { icon: '🎫', name: 'Alpine Route — Đặt vé', desc: 'Đặt trước bắt buộc mùa GW', url: 'https://www.alpen-route.com/' },
  { icon: '🚂', name: 'Kurobe Trolley — Đặt vé', desc: 'Tàu trolley hẻm núi Kurobe', url: 'https://www.kurotetu.co.jp/' },
  { icon: '🏨', name: 'HOTEL R9 The Yard Kurobe', desc: 'Khách sạn đã chốt — 2 đêm', url: 'https://hotelr9.jp/' },
  { icon: '🌦️', name: 'Dự báo thời tiết Toyama', desc: 'Kiểm tra trước khi đi', url: 'https://weather.yahoo.co.jp/weather/jp/16/' },
  { icon: '🗺️', name: 'Google Maps — Toyama', desc: 'Bản đồ khu vực', url: 'https://maps.google.com/?q=Toyama,Japan' },
  { icon: '📱', name: 'NAVITIME — Navigation', desc: 'App chỉ đường Nhật Bản', url: 'https://www.navitime.co.jp/' },
  { icon: '🚗', name: 'NEXCO — Tắc đường GW', desc: 'Check tắc đường real-time', url: 'https://www.drivetraffic.jp/' },
];

// ===== ESSENTIALS: FOOD GUIDE =====
const FOOD_GUIDE = [
  { emoji: '🦐', name: '白エビ (Shiro Ebi)', desc: 'Tôm trắng Toyama — đặc sản #1. Tempura hoặc sashimi.' },
  { emoji: '🍣', name: 'ます寿し (Masu Sushi)', desc: 'Sushi cá hồi ép truyền thống. Mua bento mang đi tuyệt vời.' },
  { emoji: '🍲', name: '五箇山豆腐', desc: 'Đậu hũ Gokayama cứng, nướng than — vị đậm đà.' },
  { emoji: '🐟', name: '岩魚塩焼き', desc: 'Cá suối nướng muối. Thơm, giòn, trẻ em thích.' },
  { emoji: '🍛', name: 'Kurobe Dam Curry', desc: 'Cà ri hình đập nước — món signature tại Kurobe Dam.' },
  { emoji: '🍜', name: '富山ブラックラーメン', desc: 'Ramen đen Toyama — nước dùng xì dầu đậm đặc.' },
  { emoji: '🦑', name: 'ほたるいか (Hotaruika)', desc: 'Mực phát sáng Toyama Bay — mùa 3-5 月. Sashimi hoặc boiled.' },
];

// ===== WEATHER DATA (static) =====
const WEATHER = [
  { location: '室堂 (2,450m)', temp: '-3 ~ 2°C', icon: '❄️', note: 'Tuyết có thể rơi' },
  { location: '黒部峡谷', temp: '14 ~ 20°C', icon: '🌿', note: 'Hẻm núi mát mẻ' },
  { location: '富山 / 魚津', temp: '15 ~ 22°C', icon: '🌤️', note: 'Ấm, mưa nhẹ' },
  { location: '松本', temp: '10 ~ 20°C', icon: '🌤️', note: 'Mát buổi sáng' },
  { location: '五箇山', temp: '13 ~ 18°C', icon: '🌤️', note: 'Ấm, có thể mưa' },
  { location: 'ヒスイ海岸', temp: '15 ~ 20°C', icon: '🌊', note: 'Gió biển mát' },
  { location: '白馬', temp: '8 ~ 20°C', icon: '🏔️', note: 'Thung lũng Alps, đêm lạnh' },
];
